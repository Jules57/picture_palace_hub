...
class CinemaHallDetailViewTest(TestCase):
    ...
    def test_get_context_data_no_shows(self):
        ...
        # Create a CinemaHall without associated MovieShows
...
class CinemaHallDeleteViewTest(TestCase):
    ...
    #     resp = self.client.get('/cinema/hall/1/delete/')
    #     self.assertEqual(resp.status_code, 200)
    #
    # def test_view_url_accessible_by_name(self):
    #     resp = self.client.get(reverse('shows:delete_hall', kwargs={'pk': 1}))
    #     self.assertEqual(resp.status_code, 200)
    #
    # def test_view_uses_correct_template(self):
    #     resp = self.client.get(reverse('shows:delete_hall', kwargs={'pk': 1}))
    #     # self.assertEqual(resp.status_code, 200)
    #     self.assertTemplateUsed(resp, 'movie_shows/halls/hall_delete.html')
...
class MovieShowListViewTest(TestCase):
    ...
    def test_get_queryset_default(self):
        ...
        # Check if the default ordering is correct
        self.assertEqual(queryset.query.order_by, ('-start_time', '-ticket_price'))

    # def test_get_queryset_sorted_by_ticket_price_asc(self):
    #     request = self.factory.get(reverse('shows:show_list'), {'sort_by': 'ticket_price', 'sort_order': 'asc'})
    #     request.user = self.user
    #
    #     view = MovieShowListView()
    #     view.setup(request)
    #
    #     queryset = view.get_queryset()
    #     print(queryset.query.order_by)
    #
    #     # Check if the queryset is sorted by ticket_price in ascending order
    #     self.assertEqual(len(queryset.query.order_by), 1)
    #     self.assertEqual(queryset.query.order_by[0], 'ticket_price')
    ...
    def test_get_context_data_default(self):
        ...
        # Check if the default context values are present
        self.assertIn('shows', context)
    ...
    def test_get_queryset_today_filter(self):
        ...
        # Check if the queryset is filtered correctly for today's shows
    ...
    def test_get_queryset_next_day_filter(self):
        ...
        # Check if the queryset is filtered correctly for the next day's shows
        self.assertEqual(queryset.count(), 1)
